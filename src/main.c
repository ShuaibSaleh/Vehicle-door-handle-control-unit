...
/*********** global ***************/
int flag_DoorUnlockState = 0;
int flag2_DoorUnlockState = 0;
...
int main() {
	...
	while (1) {
		...
		if (!doorUnlocked && !antiTheftLock) { // unlock  defualte state doorlocked
			if (!Gpio_ReadPin(GPIO_A, 1) && flag_DoorUnlockState == 0) {
				flag_DoorUnlockState = 1;
				flag2_DoorUnlockState = 1;
			}
		}

		if (flag_DoorUnlockState == 1) {
			DoorUnlockState();
		}
